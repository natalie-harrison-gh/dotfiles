...
- Git: `delta` is the diff pager; `main` is the default branch;
  rebase-by-default. Handy aliases: `g`, `git c/com/s/co/cob/l/lg`. Commit
  signing is deliberately off until Natalie configures her own key.
- Multiplexer: use `herdr` (prefix `C-;`), not tmux.
- GitHub: `gh`. JSON: `jq`. Lint shell: `shellcheck`.
